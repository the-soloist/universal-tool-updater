...
`downloads` 用于本次下载文件、断点续传分片及等待工具整体安装成功的完整产物；多产物更新中断后，已完成文件会在下次运行中校验并复用。断点缓存的 schema v2 元数据包含已确认字节数和 SHA-256；续传前会重新计算分片哈希，损坏缓存会被丢弃。`verified: transport` 仅表示 HTTP 传输完整。`staging` 用于合并、压缩和安装事务。`staging` 不能与 `state` 重叠。工具安装路径、版本文件和符号链接目标也不能与 `downloads`、`staging`、`state` 重叠。
