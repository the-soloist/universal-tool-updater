...
注意：通过 Windows 计划任务或快捷方式启动时，进程的工作目录可能漂移到 `system32` 等位置。`toolkit_root` 等相对路径以启动时的工作目录为基准解析，建议在快捷方式中设置「起始位置」（Start in），或在计划任务/配置中使用绝对路径，避免工具被安装到意外位置。
...
`toolkit_root`、`downloads` 和 `staging` 承载了更新过程中几乎全部的文件写入（下载落盘、解压输出、合并暂存与压缩），建议放在本地磁盘上不被实时监控的目录，避免放在同步盘、网络盘或被安全软件深度扫描的位置。更新器运行期间会产生大量短生命周期的临时文件，实测文件操作过滤驱动（杀软实时监控、同步盘钩子等）可带来 40% 以上的额外开销；若无法更换目录，可考虑将这几个目录加入杀软的排除列表或信任区。

即使目录被实时监控，安装结果的完整性也不受影响：

- 下载产物可配置 `sha256` pin，落盘后逐字节校验；
- 安装是事务性的，提交失败会从备份回滚原有安装；
- 解压侧阻止路径逃逸与非便携文件名，条目内容损坏会在解压校验中报错。
...
调参参考：默认的 `4` 适合网络与磁盘混合负载；以解压为主的批量更新（本地缓存已就绪、瓶颈在 CPU 与磁盘）可尝试 `6`–`8`；不建议超过逻辑核心数，过高的并发只会增加磁盘争抢与内存占用。
